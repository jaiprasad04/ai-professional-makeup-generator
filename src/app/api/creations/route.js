...
export async function GET(req) {
  try {
        ...
        try {
          ...
          if (checkRes.ok) {
            ...
            if (state === "completed" || state === "succeeded") {
              ...
              if (outputUrl) {
                creation = await prisma.makeupCreation.update({
    ...
    const creations = await prisma.makeupCreation.findMany({
          ...
          try {
            ...
            if (checkRes.ok) {
              ...
              if (state === "completed" || state === "succeeded") {
                ...
                if (outputUrl) {
                  return await prisma.makeupCreation.update({
                    where: { id: creation.id },
                    data: { status: "completed", resultImage: outputUrl }
                  });
                }
...
export async function DELETE(req) {
  try {
    ...
    const creation = await prisma.makeupCreation.findFirst({
      where: { id, userId: session.user.id }
    });
    ...
    await prisma.makeupCreation.delete({
      where: { id }
    });
